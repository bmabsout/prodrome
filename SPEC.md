...
document is the contract an implementation meets, and the crates in this
repository (`core/`, `wasm/`) are the reference implementation of it.

`conformance/*.json` are vectors generated by an earlier, independent
implementation — a Python reference, retired in 2026 — WHILE IT WAS STILL
INDEPENDENT, and they are now FROZEN: there is no generator, they are evidence
...
- **One evaluator.** Fulfillment is computed in exactly one place. Every
  consumer (a document, a web page, a phone) reads numbers the core produced.
...
never a crash: parsing is fuzzed (`core/tests/fuzz.rs`).
...
`timedelta` (`core/tests/events.rs` pins the set).
...
- JSON: `to_json`/`from_json` with the kind tags the `fpl` module declares.
...
Every implementation, on `conformance/*.json`:
